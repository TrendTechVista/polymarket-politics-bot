...
import { discoverPoliticsMarkets } from "./discovery.js";
import { runValue } from "./strategies/value.js";
import { findArbOpportunity, runArbitrage } from "./strategies/arbitrage.js";

async function run(): Promise<void> {
  const markets = await discoverPoliticsMarkets();
  if (markets.length === 0) {
    console.log("No binary politics markets found. Set POLITICS_TAG_ID or check Gamma API.");
    return;
  }
  let acted = 0;
...
async function main(): Promise<void> {
  ...
  console.log("Polymarket Politics Bot");
